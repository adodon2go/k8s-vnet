...
for kconf in ${kubeconfs[@]}; do
  ...
  scripts/nsm_install_interdomain.sh --nsm-hub="${NSM_HUB}" --nsm-tag="${NSM_TAG}" --nsm-dir="$NSMDIR" --kconf="$kconf"
done
...
for kconf in ${kubeconfs[@]}; do
  ...
  scripts/vl3_interdomain.sh \
    --kconf="$kconf" \
    --ipam-octet="$VL3_IPAMOCTET" \
    --remote-ip="$REMOTE_IPS" \
    --nse-hub="$NSE_HUB" \
