...
pushd "$SCRIPT_DIR" || (echo "directory does not exist mos likely because script was executed from PATH" && exit 1)
...
./build_and_kind_load.sh --nse-hub="tiswanso" --nse-tag="latest" --kind-clusters="kind-1 kind-2"

EXAMPLES_DIR=${EXAMPLES_DIR} KUBECONFDIR=${KUBECONFDIR} ./run_vl3_nse.sh --nse-hub="tiswanso" --nse-tag="latest" --nsm-dir="$NSM_DIR"
